...
// TODO move file to DeviceAccess.
...
namespace ChimeraTK {
  ...
  expectedUserType convertPyScalar(const py::object& input) {
    ...
    if(py::isinstance<py::int_>(clean_input)) {
      ...
      int sign = _PyLong_Sign(clean_input.ptr());
      if(sign < 0) {
  ...
  std::vector<std::vector<expectedUserType>> convertPyObject2D(const py::object& input) {
    if(py::isinstance<py::array>(input)) {
      auto arr = input.cast<py::array>();
      if(arr.ndim() != 2) {
        throw py::value_error("convertPyObject2D: expected a 2D numpy array");
      }
  ...
  expectedUserType convertScalarValue(const py::object& input) {
    ...
    if(ret.size() != 1) {
      // I would through py::value_error but a test checks for RuntimeError
      throw std::runtime_error("array of size!=1 was supplied where a scalar value was expected");
    }
    return ret[0];
  }
